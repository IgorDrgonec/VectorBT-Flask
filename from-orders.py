...
call_seq = build_call_seq(
    target_shape=(2, 2),
    group_lens=np.array([2]),
    call_seq_type=pf_enums.CallSeqType.Default
)
...
sim_out = pf_nb.simulate_from_orders_nb(
    ...
    cash_deposits=np.asarray(-np.inf),
    close=data.get("Close").values,
    size=size.values,
    size_type=np.array(pf_enums.SizeType.Percent),
    direction=np.array(pf_enums.Direction.LongOnly)
)
print_orders(symbol_wrapper.shape_2d, sim_out.order_records)
...
pf = vbt.Portfolio.from_orders(
    close=close,
    size=vbt.BCO([-np.inf, np.inf], product=True),
...
# %%
solbtc_data = vbt.BinanceData.fetch(
    "SOLBTC",
