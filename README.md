...
- [x] **Records redesign**: Every records class now has a config that describes each field of the NumPy records array. This enables many useful automatisms, such as type checks and construction of human-readable DataFrames. This also makes it far easier to subclass other records classes.
- [x] **Data refactoring**: In case of connectivity issues, data won't be lost but returned so it can be updated later. Also, symbol fetching methods can also return a state, which will be preserved for the use in data updates. There is also a new progress bar for symbols.
...
- [x] **Checks during simulation**: Typing checks and OHLC violation checks have been introduced.
...
- [x] **Shortcut properties**: Portfolio simulation based on order functions can now take user-defined arrays, write them in place during the simulation, and use them directly instead of calculating them during the analysis phase. This is the new built-in way to pre-calculate attributes such as returns. Most attributes in Portfolio and Records have also become properties, while their respective methods now have a "get_" prefix.
- [x] **Config optimizations**: Config classes have been optimized. The time overhead of using the top-level API has shrunk by 25%.
