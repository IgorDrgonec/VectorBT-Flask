...
- [x] **Multiprocessing**: Integration of the Ray backend for running multiple chunks across multiple processes. Ever wanted to test billions of hyperparameter combinations in a matter of minutes? This is now possible by scaling any application and even function up in the cloud using the Ray cluster.
...
- [x] **Meta methods**: Many methods such as rolling apply are now available in two flavors: regular (instance methods) and meta (class methods). Regular methods look as usual, while meta methods are not bound to any array and act as micro-pipelines with their own broadcasting and templating.
- [x] **Rolling metrics**: Rolling metrics based on returns have been optimized for best performance (up to 100x speedup).
- [x] **Flexible portfolio attributes**: Portfolio attributes such as asset flow now allow overriding other attributes they are based on. This allows great control of post-simulation analysis phase, for example, to reduce re-calculation.
