...
- Completely refactored [Portfolio.from_signals](/api/portfolio/base/#vectorbtpro.portfolio.base.Portfolio.from_signals):
    - Signal function `signal_func_nb` takes the context [SignalContext](/api/portfolio/enums/#vectorbtpro.portfolio.enums.SignalContext) 
    with much more useful information than previously. User-defined arguments are not included in the context
    but you can always forward them to the function using templates.
    - Preset signal functions take as argument an adjustment function `adjust_func_nb` that allows changing any 
    current information prior to returning the signals, which is especially useful for adjusting limits and stops
    - When there is no signal in any of the columns of a group, the group won't be executed, which
    results in a great speedup for sparsely-distributed signals
    ...
    - Added an option for specifying the format of any delta (`delta_format` of type [DeltaFormat](/api/portfolio/enums/#vectorbtpro.portfolio.enums.DeltaFormat)),
    which makes possible providing stop values in absolute terms
    - Information on each stop order is now stored in one record array - one record per column, not separate arrays. 
    For example, stop loss information is represented by a record array of data type 
    [sl_info_dt](/api/portfolio/enums/#vectorbtpro.portfolio.enums.sl_info_dt).
    - Added limit order support. Whenever a signal of the limit order type is placed, it's first checked
    against the current bar information (OHLC if signal was placed at open, only C otherwise).
    It's then stored in a record array of type [limit_info_dt](/api/portfolio/enums/#vectorbtpro.portfolio.enums.limit_info_dt)
    that gets propagated throughout time and checked whether the limit has been hit or expired at each bar.
    Only one active limit order is allowed at a time.
    - Similarly to stop orders, a limit delta can be specified to represent a distance between the initial
    price (`price`) and the target price. For example, a delta of 0.1 will place a limit order with the target 
    price of 10% above/below the initial price when selling/buying.
    ...
    [TimeDeltaFormat](/api/portfolio/enums/#vectorbtpro.portfolio.enums.TimeDeltaFormat).
    If the format is based on index, time deltas and expiry times will be converted into integer, nanosecond
    format. If the input is a timedelta/datetime-like Series/DataFrame, the pre-processing will remove
    the timezone and convert to integer format automatically.
    ...
    and execute the user-defined signal if it's sign is opposite to the limit order's sign.
    The same for pending stop orders, which can be cancelled by executable limit/user signals.
    - Order records generated by [Portfolio.from_signals](/api/portfolio/base/#vectorbtpro.portfolio.base.Portfolio.from_signals)
    were extended with the timestamp of signal (`signal_idx`), timestamp of (stop/limit) order creation 
    (`creation_idx`), order type (`type`), and stop type (`stop_type`). The new data type is 
    [fs_order_dt](/api/portfolio/enums/#vectorbtpro.portfolio.enums.fs_order_dt). The new order type can be 
    analyzed using [FSOrders](/api/portfolio/trades/#vectorbtpro.portfolio.orders.FSOrders), which is a 
    subclass of [Orders](/api/portfolio/trades/#vectorbtpro.portfolio.orders.Orders). Other simulation methods,
    such as [Portfolio.from_orders](/api/portfolio/base/#vectorbtpro.portfolio.base.Portfolio.from_orders),
    still generate records of the type [order_dt](/api/portfolio/enums/#vectorbtpro.portfolio.enums.order_dt).
- [Portfolio](/api/portfolio/base/#vectorbtpro.portfolio.base.Portfolio) allows providing custom record classes. 
For example, one can create a custom subclass of [Trades](/api/portfolio/trades/#vectorbtpro.portfolio.trades.Trades)
and pass it as `trades_cls` for portfolio to use it by default.
- Added `ValuePercent` - a new size type for ordering a percentage of the current group value. 
But also, `Percent`, `ValuePercent`, and `TargetPercent` have alternatives for provision of percentages 
in a human-readable format (1.0 = 1%).
- Instead of passing `-np.inf` or `np.inf` as order/valuation price to represent open/latest and close/price,
enumerated values [PriceType](/api/portfolio/enums/#vectorbtpro.portfolio.enums.PriceType)/
[ValPriceType](/api/portfolio/enums/#vectorbtpro.portfolio.enums.ValPriceType) can be used.
For example, to specify that the opening price should be used as order price, pass `price="open"`.
- [QSAdapter](/api/returns/qs_adapter/#vectorbtpro.returns.qs_adapter.QSAdapter) will also remove the timezone
from benchmark returns, and will force the user to select one column (by offering a `column` argument) 
since quantstats can use only one benchmark column at a time.
- OHLC chart ([OHLCVDFAccessor.plot](/api/ohlcv/accessors/#vectorbtpro.ohlcv.accessors.OHLCVDFAccessor.plot)) 
has been updated to match the style of TradingView
- Specifying the filename when saving or loading any vectorbt object is no longer necessary - 
vectorbt will use the class name by default
...
- Implemented [TVData](/api/data/custom/#vectorbtpro.data.custom.TVData), which wraps [tvdatafeed](https://github.com/StreamAlpha/tvdatafeed)
