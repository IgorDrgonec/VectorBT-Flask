...
```python-repl
>>> from vectorbtpro.jit_registry import jit_registry, JitableSetup
...
```python-repl
>>> jit_registry.jitable_setups['vectorbtpro.generic.nb.diff_nb']['nb'].jitter_kwargs
{'cache': True}
```
...
>>> # disabled
>>> jit_registry.resolve('vectorbtpro.generic.nb.diff_nb', jitter='nb').targetoptions
...
class JITRegistry:
    ...
    def resolve(self,
        ...
        * `jitters.your_jitter.resolve_kwargs` in `vectorbtpro._settings.settings`
        * `jitters.your_jitter.tasks.your_task.resolve_kwargs` in `vectorbtpro._settings.settings`
        * `jitter_kwargs`
...
def register_jitted(py_func: tp.Optional[tp.Callable] = None,
    ...
    * `jitters.{jitter_id}.options` in `vectorbtpro._settings.settings`
    * `jitters.{jitter_id}.tasks.{task_id}.options` in `vectorbtpro._settings.settings`
    * `options`
    * `jitters.{jitter_id}.override_options` in `vectorbtpro._settings.settings`
    * `jitters.{jitter_id}.tasks.{task_id}.override_options` in `vectorbtpro._settings.settings`

    `py_func` can also be overridden using `jitters.your_jitter.tasks.your_task.replace_py_func`
    in `vectorbtpro._settings.settings`."""
