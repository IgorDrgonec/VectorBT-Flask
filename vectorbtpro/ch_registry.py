...
def register_chunkable(func: tp.Optional[tp.Callable] = None,
    ...
    * `chunking.options` in `vectorbtpro._settings.settings`
    * `chunking.setup_options.{setup_id}` in `vectorbtpro._settings.settings`
    * `options`
    * `chunking.override_options` in `vectorbtpro._settings.settings`
    * `chunking.override_setup_options.{setup_id}` in `vectorbtpro._settings.settings`

    !!! note
        Calling the `register_chunkable` decorator before (or below) the `vectorbtpro.jit_registry.register_jitted`
        decorator with `return_wrapped` set to True won't work. Doing the same after (or above)
        `vectorbtpro.jit_registry.register_jitted` will work for calling the function from Python but not from Numba.
