...
class TestAccessors:
    def test_vwap(self):
        pd.testing.assert_series_equal(
            ...
            ((ohlcv_ts['volume'] * (ohlcv_ts['high'] + ohlcv_ts['low']) / 2).cumsum() /
            ohlcv_ts['volume'].cumsum()).rename('vwap')
        )
        result = pd.concat((
            ohlcv_ts.vbt.ohlcv.vwap(),
            ohlcv_ts.vbt.fshift(1).vbt.ohlcv.vwap()
        ), axis=1)
        result.columns = ['a', 'b']
        ...
        pd.testing.assert_frame_equal(
            pd.DataFrame.vbt.ohlcv.vwap(high=high, low=low, volume=volume),
            result
        )
